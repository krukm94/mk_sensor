...
void afeInit(void)
{
	...
	/**AFE   RESETZ PIN
	*/
	GPIO_InitStruct.Pin = PPG_RES_PIN;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM;
	HAL_GPIO_Init(PPG_RES_PORT, &GPIO_InitStruct);
	
	/**AFE   ADC RDY PIN 
	*/
	GPIO_InitStruct.Pin = PPG_ARDY_PIN;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	HAL_GPIO_Init(PPG_ARDY_PORT, &GPIO_InitStruct);
	
	HAL_NVIC_SetPriority(EXTI15_10_IRQn, AFE_ADC_RDY_PRIORITY , 0);
  HAL_NVIC_EnableIRQ(EXTI15_10_IRQn);
	
			/**I2C1 GPIO Configuration  I2C FOR AFE   
	*/
	GPIO_InitStruct.Pin = PPG_I2C_SCL_PIN|PPG_I2C_SDA_PIN ;
	GPIO_InitStruct.Mode = GPIO_MODE_AF_OD;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	GPIO_InitStruct.Alternate = GPIO_AF4_I2C1;
	HAL_GPIO_Init(PPG_I2C_SCL_PORT, &GPIO_InitStruct);		//PPG_I2C_SCL_PORT is the same as PPG_I2C_SDA_PORT
	...
	//SET RESETZ HIGH
	LL_GPIO_SetOutputPin(PPG_RES_PORT , PPG_RES_PIN );
  ...
  if (HAL_I2CEx_ConfigDigitalFilter(&hi2c1, 0) != HAL_OK)
  {
	...
	//Hardware Reset
	LL_GPIO_ResetOutputPin(PPG_RES_PORT , PPG_RES_PIN);
	delay_us_mk(30);
	LL_GPIO_SetOutputPin(PPG_RES_PORT , PPG_RES_PIN);
	...
	*sign = 0x02;
	...
	char* printbuf = malloc(100);	// Alloc mem for Print Buf
		...
		if(nrOfLed == 1) memAddr = (uint16_t)LED1VAL; 	//Check nrOfLed and set proper registwr address 
			...
			ServUsart->writeString(ServUsart->usartHandle , printbuf);
...
/**
  * @brief  Avregaging
  */
void sre_afe(float *wynik){

		srednia = srednia * dt_afe;
		srednia = srednia + *wynik;
		srednia = srednia /(dt_afe+1);
}
...
void sre2(int32_t *wynik){
		...
		srednia2 = srednia2 * dt_afe;
		srednia2 = srednia2 + *wynik;
		srednia2 = srednia2 /(dt_afe+1);
}
	...
	while(loop_cnt != 51){
			...
			if(ret_val == 0){
					...
					if(check_val != afrReg_Val[loop_cnt][1]){
						...
						sprintf(buf , "$$$ AFE: read Adr nr: %d  OK!\r\n" , loop_cnt);
						ServUsart->writeString(ServUsart->usartHandle , buf);
					}
			}
			else{
